              ...
              {data.map((item, index) => (
                <tr key={index}>
                 <td>{item.tokenAddress.substring(0, 6)}...{item.tokenAddress.substring(item.tokenAddress.length - 4)}  <Tooltip title="copy">  <button onClick={() => copyToClipboard(item.tokenAddress)}><FontAwesomeIcon icon={faCopy} /></button></Tooltip></td>
                  ...
                  <td>{item.contractAddress.substring(0, 6)}...{item.contractAddress.substring(item.contractAddress.length - 4)} <Tooltip title="copy"> <button onClick={() => copyToClipboard(item.contractAddress)}><FontAwesomeIcon icon={faCopy} /></button></Tooltip></td>
                ...
                <Tooltip title="Click to view proposal details">
                    ...
                    <button  className="hover:font-bold" onClick={() => openModal(item)}>View <strong>
                    ↗ </strong> </button>
                </Tooltip>
                  </td>
                  <td>
